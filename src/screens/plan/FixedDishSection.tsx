...
interface Props {
  ...
  /** 日ごとのメンバー(ゲストを含む)。注意の計算に使う */
  days: readonly DayInput[];
  conditions: PlanConditions;
  data: PlannerData;
}

/** 条件の画面の「料理を指定」:何日目のどの区分に、どのレシピを入れるか */
  ...
  return (
    <>
      ...
      <p className="field-hint" style={{ marginTop: 0 }}>
        食べたい料理を、何日目の主菜・副菜・汁物に入れるか決められます。残りの品はアプリが考えます。
      </p>
      {fixed.map((f) => {
        ...
        return (
          <div key={`${f.dayIndex}-${f.course}`} className="card form" style={{ gap: 6 }}>
            <div className="btn-row" style={{ alignItems: 'center' }}>
              <span style={{ flex: 1 }}>
                {f.dayIndex + 1}日目の{f.course}:{recipe?.name ?? '(消えたレシピ)'}
              </span>
      ...
      <div className="card form" style={{ gap: 12 }}>
        ...
        <button type="button" className="btn" onClick={() => setPicking(true)}>
          {slot.dayIndex + 1}日目の{slot.course}を選ぶ
        </button>
      </div>

      {picking && (
        <RecipePickerSheet
          title={`${slot.dayIndex + 1}日目の${slot.course}を選ぶ`}
          course={slot.course}
